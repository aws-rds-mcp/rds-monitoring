...
class TestGetCloudWatchClient:
    """Tests for the get_cloudwatch_client function."""

    def test_get_cloudwatch_client_default(self, mock_boto3):
        """Test getting a CloudWatch client with default parameters."""
        client = get_cloudwatch_client()
        mock_boto3['client'].assert_called_once_with('cloudwatch', region_name='us-east-1')
        assert client == mock_boto3['cloudwatch']

    def test_get_cloudwatch_client_with_region(self, mock_boto3):
        """Test getting a CloudWatch client with a specific region."""
        client = get_cloudwatch_client(region_name='us-east-1')
        mock_boto3['client'].assert_called_once_with('cloudwatch', region_name='us-east-1')
        assert client == mock_boto3['cloudwatch']

    def test_get_cloudwatch_client_with_profile(self, mock_boto3):
        """Test getting a CloudWatch client with a specific profile."""
        client = get_cloudwatch_client(profile_name='test-profile')
        mock_boto3['Session'].assert_called_once_with(profile_name='test-profile')
        mock_boto3['Session'].return_value.client.assert_called_once_with(
    ...
    def test_get_cloudwatch_client_with_region_and_profile(self, mock_boto3):
        """Test getting a CloudWatch client with a specific region and profile."""
        client = get_cloudwatch_client(region_name='us-east-1', profile_name='test-profile')
        mock_boto3['Session'].assert_called_once_with(profile_name='test-profile')
        mock_boto3['Session'].return_value.client.assert_called_once_with(
            'cloudwatch', region_name='us-east-1'
        )
    ...
    def test_get_cloudwatch_client_with_none_region(self, mock_boto3):
        """Test getting a CloudWatch client with None region."""
        client = get_cloudwatch_client(region_name=None)
        mock_boto3['client'].assert_called_once_with('cloudwatch', region_name='us-east-1')
        assert client == mock_boto3['cloudwatch']
